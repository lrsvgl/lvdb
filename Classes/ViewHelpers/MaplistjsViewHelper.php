...
class MaplistjsViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractTagBasedViewHelper
{
	/**
	...
	public function render($locations, $city)
	{
		...
		while ($locations->valid()) {
			...
			var_dump($locations->current()->getTitle());
			$lat = $locations->current()->getLat();
			$lon = $locations->current()->getLon();
...
//		}
	...
	function getMapJavascript($locations)
	{
		...
		function load(){
		    ...
		    var latlng = new google.maps.LatLng(' . $locations->getLat() . ',' . $locations->getLon() . ');
		     myOptions = {
		      zoom: zoom1,
		      center: latlng,
